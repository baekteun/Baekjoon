...
fun main(args: Array<String>)=with(BufferedReader(InputStreamReader(System.`in`))){
    ...
    while(true){
        ...
        if(N==0) break
        var map = Array<IntArray>(N) { IntArray(N) } //동굴
        var visited = Array<BooleanArray>(N) { BooleanArray(N) }//좌표방문 OX
        var rupee = Array<IntArray>(N) { IntArray(N) } //다익스트라 결과
        for(i in 0 until N){
